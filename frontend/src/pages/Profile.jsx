...
export default function Profile() {
  ...
  const loadCampaigns = async () => {
    if (principal) {
      try {
        const campaignsData = await backendActor.getCampaignsByUser(
          Principal.fromText(principal)
        );
  ...
  const loadDonations = async () => {
    if (principal) {
      try {
        const donationsData = await backendActor.getDonationsByUser(
          Principal.fromText(principal)
        );
                ...
                <>
                  <ul>
                    {paginatedCampaigns.map((camp, index) => (
                      <li
                        key={index}
                        className="bg-gray-50 border-l-4 border-blue-500 p-4 rounded-lg shadow-md mb-4"
                      >
                        <p className="text-lg font-semibold">{camp.title}</p>
                        <p className="text-gray-600">{camp.description}</p>
                        <p className="text-sm text-gray-500">
                          <strong>Collected:</strong>{" "}
                          {camp.collected.toString()} / {camp.target.toString()}{" "}
                          ICP
                        </p>
                        <p className="text-sm text-gray-500">
                          <strong>Date:</strong>{" "}
                          {new Date(
                            Number(camp.date) / 1_000_000
                          ).toLocaleDateString()}
                        </p>
                        <p className="text-sm text-gray-500">
                        ...
                        <div className="mt-4 flex items-center space-x-4">
                          <input
                            type="file"
                            onChange={(e) =>
                              handleCampaignFileUpload(camp.id, e)
                            }
                ...
                <>
                  <ul>
                    {paginatedDonations.map((don, index) => (
                      <li
                        key={index}
                        className="bg-gray-50 border-l-4 border-green-500 p-4 rounded-lg shadow-md mb-4"
                      >
